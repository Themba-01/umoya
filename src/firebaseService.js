...
// ============= SHARED GAME HELPERS (10 positions) =============
const ADJ = {
  1: [2],
  2: [1, 3, 5],
  3: [2],
...
const WINNING_LINES = [
  ...
  [2, 5, 6],     // upper vertical
  [5, 6, 9]      // lower vertical
];
...
function normalizeBoard(board) {
  ...
  // If it's an array (Firebase coercion), convert back to object with numeric keys 1-10
  if (Array.isArray(board)) {
    console.warn('⚠️ Board was an array! Converting to object.');
    const newBoard = {};
    for (let i = 1; i <= 10; i++) {
