...
// ============= GAME LOGIC =============
const POS = {
  1: { x: 110, y: 140 },   // top left
  2: { x: 310, y: 140 },   // top center
  3: { x: 510, y: 140 },   // top right
  4: { x: 110, y: 240 },   // left middle arm end
  5: { x: 310, y: 240 },   // left arm attachment on vertical (higher)
  6: { x: 310, y: 340 },   // right arm attachment on vertical (lower)
  7: { x: 510, y: 340 },   // right middle arm end
  8: { x: 110, y: 440 },   // bottom left
  9: { x: 310, y: 440 },   // bottom center
  10: { x: 510, y: 440 },  // bottom right
};
...
const WINNING_LINES = [
  ...
  [8, 9, 10],    // bottom horizontal
  [2, 5, 6],     // upper vertical (3 in a row)
  [5, 6, 9]      // lower vertical (3 in a row)
];
...
// Heuristic weights for positional evaluation
const POSITION_WEIGHTS = {
  5: 10, 6: 10,   // central vertical attachments
  2: 8, 9: 8,     // top/bottom centers
  4: 8, 7: 8,     // middle arm ends
  1: 5, 3: 5, 8: 5, 10: 5   // corners
};
  ...
  return (
    <div className="game-board-container">
      ...
      >
        ...
        {/* Left middle arm - horizontal at higher position */}
        <line x1="110" y1="240" x2="310" y2="240" stroke="#67e8f9" strokeWidth="28" strokeLinecap="round" />
        
        {/* Right middle arm - horizontal at lower position */}
        <line x1="310" y1="340" x2="510" y2="340" stroke="#67e8f9" strokeWidth="28" strokeLinecap="round" />
        
        {/* Bottom horizontal */}
        <line x1="110" y1="440" x2="510" y2="440" stroke="#67e8f9" strokeWidth="28" strokeLinecap="round" />
        
        {/* Full vertical through all 4 vertical positions */}
        <line x1="310" y1="140" x2="310" y2="440" stroke="#67e8f9" strokeWidth="28" strokeLinecap="round" />
      </svg>
        ...
        <div className={`player-card ${playerSymbol === 'X' ? 'active' : ''} ${gameState.currentPlayer === 'X' ? 'turn' : ''}`}>
          ...
          <div className="player-info">
            <div className="player-name">
              {gameState.players?.find(p => p?.symbol === 'X')?.name || 'Player X'}
            </div>
            <div className="player-pieces">{gameState.placementCount?.X ?? 0}/3 placed</div>
          </div>
        </div>
        ...
        <div className={`player-card ${playerSymbol === 'O' ? 'active' : ''} ${gameState.currentPlayer === 'O' ? 'turn' : ''}`}>
          ...
          <div className="player-info">
            <div className="player-name">
              {isVsAI 
                ? '🤖 AI Bot' 
                : (gameState.players?.find(p => p?.symbol === 'O')?.name || 'Waiting...')}
            </div>
            <div className="player-pieces">{gameState.placementCount?.O ?? 0}/3 placed</div>
          </div>
        </div>
    ...
    <div className="menu-screen">
      <div className="menu-content">
        ...
        <div className="how-to-play">
          ...
          <p style={{marginTop: '1rem', fontSize: '0.85rem', color: '#a1a9c3'}}>
            {/* 10-position staggered board — exactly as requested */}
          </p>
        </div>
      </div>
    </div>
